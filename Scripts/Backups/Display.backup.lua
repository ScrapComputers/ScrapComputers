...
function Display:sv_createData()
    ...
    local function drawCricle(x, y, radius, color, isFilled)
        ...
        -- Check if the radius is in-bounds.
        assert(x + round(radius) <= self.data.width and x - round(radius) + 1 > 0, "bad argument #1, Out of bounds (Radius too big)")
        assert(y + round(radius) <= self.data.height and y - round(radius)  + 1 > 0, "bad argument #2, Out of bounds (Radius too big)")
        ...
        table.insert(self.sv.buffer, {
            ...
            {
                x = math.floor(x) + 1, -- The x cordinates
                y = math.floor(y) + 1, -- The y cordiantes

                radius = radius - 0.5, -- The radius of the circle
...
-- Draws a circle, filled or not, uses draw pixel to draw final pixels
...
function Display:cl_drawCircle(params)
    ...
    -- Iterate over the circle's circumference
    while cx < cy do
        if f >= 0 then
            cy = cy - 1
            ddF_y = ddF_y + 2
