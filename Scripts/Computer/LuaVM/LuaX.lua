...
function luaX:read_string(ls, del, Token)
	...
	self:save_and_next(ls) -- Start by saving the first character and moving to the next
	local buffer = {}    -- Use a table as a buffer to collect characters
	local cacheIndex = 0
	local currIsNewline = self.currIsNewline
	local nextc = self.nextc
	local lexerror = self.lexerror

	-- Iterate until end of string or del is found
	while true do
		local char = ls.current
		if char == del then
			break
		elseif char == "EOZ" or currIsNewline(self, ls) then
			lexerror(self, ls, "unfinished string", char == "EOZ" and "TK_EOS" or "TK_STRING")
			return
		elseif char == "\\" then
			char = nextc(self, ls)
			if currIsNewline(self, ls) then
				cacheIndex = cacheIndex + 1
				buffer[cacheIndex] = "\n"
				self:inclinenumber(ls)
			elseif char ~= "EOZ" then
				local readstring_escapeCode = readstring_escapeCodeLT[char]
				if readstring_escapeCode then
					cacheIndex = cacheIndex + 1
					buffer[cacheIndex] = readstring_escapeCode
					nextc(self, ls)
				elseif not string_find(char, "%d") then
					cacheIndex = cacheIndex + 1
					buffer[cacheIndex] = char
					nextc(self, ls)
				else
					local c, i = 0, 0
					repeat
						...
						nextc(self, ls)
						i = i + 1
					until i >= 3 or not string_find(ls.current, "%d")
					if c > 255 then
						lexerror(self, ls, "escape sequence too large", "TK_STRING")
						return
					end
					endcacheIndex = cacheIndex + 1
					buffer[cacheIndex] = string_char(c)
					nextc(self, ls)
				end
			end
		else
			cacheIndex = cacheIndex + 1
			buffer[cacheIndex] = char -- Add regular character to the buffer
			nextc(self, ls)
		end
	end

	self:save_and_next(ls)             -- Skip the closing delimiter
	Token.seminfo = table.concat(buffer) -- Join buffer contents into a final string
end
