...
function TextCodec:encode(data, isEncrypted, password)
    ...
    if isEncrypted then
        data = sm.scrapcomputers.aes256.encrypt(password, data, sm.scrapcomputers.aes256.AES256)
    end
...
function TextCodec:decode(data, isEncrypted, password)
    ...
    if isEncrypted then
        data = sm.scrapcomputers.aes256.decrypt(password, data, sm.scrapcomputers.aes256.AES256)
    end
