...
function sc.envManager.createEnv(self)
    local env = {
        ...
        sc = {
            ...
            getSeatControllers = function () return sc.getObjects(sc.filters.dataType.SeatControllers, self.interactable, false ) end,

            ---Gets a addon component! I hate Scrap Mechanic!
            ---@param str string
            ---@param viaParent boolean
            getAddonComponent  = function ( str, viaParent )
                assert(type(str) == "string", "bad argument #1. Expected string, Got "..type(str).." instead!")
                local viaParent = viaParent or false

                assert(type(viaParent) == "boolean", "bad argument #2. Expected boolean or nil, Got "..type(viaParent).." instead!")

                ---@type Interactable[]
                local addonComponents = self.interactable[viaParent == true and "getParents" or "getChildren"](self.interactable, sm.interactable.connectionType.compositeIO)
                if #addonComponents > 0 then
                    for _, addonComponent in pairs(addonComponents) do
                        local scPublicData = addonComponent.publicData.SCRAPCOMPUTERS_PUBLIC_DATA

                        -- Idiot Proof
                        if type(scPublicData         )          == "table"        and
                           type(scPublicData.dataType)          == "string"       and
                           scPublicData.dataType:lower()        == str:lower()    and
                           type(scPublicData.getEnv)            == "function"     and
                           type(scPublicData.self)              == "table"        and
                           type(scPublicData.self.interactable) == "Interactable" then
                            return scPublicData.getEnv(scPublicData.self), nil
                        end
                    end
                end
