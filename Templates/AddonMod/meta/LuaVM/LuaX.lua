  ...
  while true do
    ...
    elseif char == "\\" then
      ...
      if currIsNewline(self, ls) then
        ...
        local escapeValue = readstring_escapeCodeLT[char]
        if escapeValue then
          cacheIndex = cacheIndex + 1
          buffer[cacheIndex] = escapeValue
          nextc(self, ls)
        elseif char >= '0' and char <= '9' then
          -- Parse numeric escape sequence
          local c = 0
          for _ = 1, 3 do
            local current = ls.current
            if current < '0' or current > '9' then break end
            c = 10 * c + (current - '0')
            nextc(self, ls)
          end
          ...
          cacheIndex = cacheIndex + 1
          buffer[cacheIndex] = string.char(c)
        end
      end
    else
      cacheIndex = cacheIndex + 1
